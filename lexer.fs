...
module lexer =
  ...
  // Returns id if it is within keyword. 
  ...
  let rec private collectIntLiteral nextc input literal typeOfLiteral = 
    if (List.contains nextc ['0'..'9']) then
      collectIntLiteral (nextChar input) input (literal + (string nextc)) typeOfLiteral
    else if (List.contains nextc ['.']) then
      collectIntLiteral (nextChar input) input (literal + (string nextc)) "real"
    else
      (literal,nextc, typeOfLiteral)
  ...
  let rec private lexer nextc input tokens = 
      ...
      |  _  when List.contains nextc ['0'..'9'] ->
               ...
               let (literal,lookahead, typeOfLiteral) = collectIntLiteral (nextChar input) input (string nextc) ""
               if typeOfLiteral = "real" then
